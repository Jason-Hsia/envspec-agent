...
🔗 **在线 Demo（免安装，浏览器点开即用）：<https://envspec-agent.app.workbuddy.host/>**
...
| 想看什么 | 打开 |
|---|---|
| **在线 Demo**（免安装，点开就看完整链路） | <https://envspec-agent.app.workbuddy.host/> |
| **离线单文件版**（双击即开，零依赖、零后端） | `web/index.html` |
...
打开 <https://envspec-agent.app.workbuddy.host/> —— 这是 `web/index.html` 的线上镜像（SHA256 与仓库内文件一致），无需 clone、无需配环境、无需后端。适合快速评审。
...
> 左上角徽章会明示当前引擎是「真实后端」还是「内置离线引擎」。在线版与本地单文件版都会显示后者 —— 两者跑的就是同一份内置引擎，它与 Python 后端的一致性由 `tests/parity_check.py` 逐题校验（24 题 × 7 字段）。所以在线 Demo 展示的结果，与评测报告里的数字是同一套。
...
MIT License，全文见 [`LICENSE`](LICENSE)。代码可自由使用、修改、分发，保留版权声明即可。`data/` 下的标准数值为演示数据，**不可直接用于实际合规判定**。
